...
def begin(state, loaders):
  ...
  state.update(
    {
      'title': 'curriculum1_wgan_faceparsing'
    }
  )
  ...
  G_state = torch.load('/home/s2125048/thesis/model/20200711_222849/curriculum1_wgan_faceparsing/epoch0_G.pt',map_location='cuda:0' if torch.cuda.is_available() else 'cpu')
  net_G.load_state_dict(G_state)
  net_G.train()

  D_state = torch.load('/home/s2125048/thesis/model/20200711_222849/curriculum1_wgan_faceparsing/epoch0_D.pt',map_location='cuda:0' if torch.cuda.is_available() else 'cpu')
    ...
    for current_batch_index,(ground,mask,segment) in enumerate(train_loader):
      ...
      if current_batch_index % update_G_every_batch == 0 and current_batch_index > 0:
        ...
        g_loss = g_adv_loss + recon_global_loss + 10 * recon_local_loss + \
                 0.1 * g_face_parsing_loss + g_style_loss + g_tv_loss
