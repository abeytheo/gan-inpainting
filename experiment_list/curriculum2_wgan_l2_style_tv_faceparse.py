...
def begin(state, loaders):
  ...
  state.update(
    {
      'title': 'curriculum1_wgan_style_tv'
    }
  )
  ...
  G_state = torch.load('/home/s2125048/thesis/model/20200714_175249/curriculum1_wgan_style_tv/epoch45_G.pt',map_location='cuda:0' if torch.cuda.is_available() else 'cpu')
  net_G.load_state_dict(G_state)
  net_G.train()

  D_state = torch.load('/home/s2125048/thesis/model/20200714_175249/curriculum1_wgan_style_tv/epoch45_D.pt',map_location='cuda:0' if torch.cuda.is_available() else 'cpu')
  ...
  for epoch in range(num_epochs + 1):
    ...
    for current_batch_index,(ground,mask,segment) in enumerate(train_loader):
      ...
      if current_batch_index % update_G_every_batch == 0 and current_batch_index > 0:
        ...
        g_loss = g_adv_loss + recon_global_loss + 10 * recon_local_loss + g_style_loss + g_tv_loss + 0.1 * g_face_parsing_loss
