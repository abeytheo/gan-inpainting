...
def begin(state, loaders):
  ...
  state.update(
    {
      'title': 'curriculum1_wgan_percept_tv'
    }
  )
  ...
  for epoch in range(num_epochs + 1):
    ...
    for current_batch_index,(ground,mask,segment) in enumerate(train_loader):
      ...
      if current_batch_index % update_G_every_batch == 0 and current_batch_index > 0:
        ...
        g_perceptual_loss_out = loss.perceptual_loss(out,ground,weight=0.01)
        ...
        g_loss = g_adv_loss + recon_global_loss + 10 * recon_local_loss + g_perceptual_loss + g_tv_loss
